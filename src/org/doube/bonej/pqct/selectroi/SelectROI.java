...
public class SelectROI extends RoiSelector {
	...
	public SelectROI(final ScaledImageData dataIn, final ImageAndAnalysisDetails detailsIn, final ImagePlus imp,
		...
		final double[] tempScaledImage = Arrays.copyOf(scaledImage, scaledImage.length);// (double[])
																						// scaledImage.clone();
		if (ijROI != null
				&& details.manualRoi) { /*
		...
		try {
			...
			if (setRoi) {
				...
				/*
				 * Flip the original image prior to adding the ROI, if scaled
				 * image is flipped
				 */
				if ((details.flipHorizontal || details.flipVertical) && imp.getRoi() != null) {
					IJ.run(imp, "Select None", ""); // Remove existing ROIs in
													// order to flip the whole
													// image...
				}
