...
public abstract class RoiSelector {
	...
	public Vector<Object> getSieve(final double[] tempScaledImage, final double boneThreshold, final String roiChoice,
		...
		if (edges.size() < 1) {
			throw new ExecutionException(
					"Couldn't find a bone. The range of intensities in the file is " + min(tempScaledImage) + " to "
							+ max(tempScaledImage) + ". Set the Thresholds between these values",
					new Throwable());
		}
		 ...
		 * System.out.println("RoiChoice "+roiChoice+" selection "
		 * +selection+" lengthEdges "+edges.size());
		 * 
		 * System.out.println("RoiChoice "+roiChoice+" selection "
		 * +selection+" lengthEdges "+edges.size());
		 */
	...
	int selectRoiCentralBone(final Vector<DetectedEdge> edges, final double[] tempScaledImage,
		...
		final double[] temp = Arrays.copyOf(distanceFromCentreOfLimb, distanceFromCentreOfLimb.length);// (double[])
																										// distanceFromCentreOfLimb.clone();
		Arrays.sort(temp);
		int counter = 0;
	...
	public byte[] fillSieve(final Vector<Integer> roiI, final Vector<Integer> roiJ, final int width, final int height,
		...
		while (true) {
			...
			final byte[] sieveTemp2 = Arrays.copyOf(sieveTemp, sieveTemp.length);// (byte[])
																					// sieveTemp.clone();
			boolean noLeak = true;
			...
			if (noLeak) {
				sieveTemp = Arrays.copyOf(sieveTemp2, sieveTemp2.length);// (byte[])
																			// sieveTemp2.clone();
			}
		}
		// return sieveTemp;
	...
	Vector<Object> traceEdge(final double[] scaledImage, final byte[] result, final double threshold, int i, int j) {
		...
		while (true) {
			...
			// IJ.log("Init traceEdge W "+width+" H "+height+" i "+i+" j "+j+"
			// check i "+(i+((int)
			// Math.round(Math.cos(direction-Math.PI/4.0))))+" check j
			// "+(j+((int) Math.round(Math.sin(direction-Math.PI/4.0))))+" bool
			// "+((j+((int) Math.round(Math.sin(direction))) >=0)));
			if ((i + ((int) Math.round(Math.cos(direction)))) >= 0
					...
					&& scaledImage[i + ((int) Math.round(Math.cos(direction)))
							+ (j + ((int) Math.round(Math.sin(direction)))) * width] > threshold) {// Rotate
																									...
																									// i
																									// "+(i+((int)
																									// Math.round(Math.cos(direction-Math.PI/4.0))))+"
																									// j
																									// "+(j+((int)
																									// Math.round(Math.sin(direction-Math.PI/4.0)))));
				while (counter < 8 && i + ((int) Math.round(Math.cos(direction - Math.PI / 4.0))) >= 0
						...
						&& scaledImage[i + ((int) Math.round(Math.cos(direction - Math.PI / 4.0)))
					...
					// IJ.log("traceEdge rotating counter W "+width+" H
					// "+height+" i "+i+" j "+j);
				}
			} else {// Rotate clockwise
					// IJ.log("traceEdge rotate clock W "+width+" H "+height+" i
					// "+i+" j "+j);
				while (counter < 8 && (i + ((int) Math.round(Math.cos(direction))) < 0
						|| i + ((int) Math.round(Math.cos(direction))) >= width
						|| j + ((int) Math.round(Math.sin(direction))) < 0
						|| j + ((int) Math.round(Math.sin(direction))) >= height
						|| scaledImage[i + ((int) Math.round(Math.cos(direction)))
					...
					// IJ.log("traceEdge rotating clock W "+width+" H "+height+"
					// i "+i+" j "+j);
				}
	...
	Vector<Object> fillResultEdge(byte[] result, final Vector<Integer> iit, final Vector<Integer> jiit,
		...
		if (iit.size() > 0) {
			...
			final byte[] tempResult = Arrays.copyOf(result, result.length);// (byte[])
																			// result.clone();
			...
			if (possible) {
				result = Arrays.copyOf(tempResult, tempResult.length);// tempResult.clone();
