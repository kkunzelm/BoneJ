...
public class SelectSoftROI extends RoiSelector {
		...
		if (details.stOn) {
			...
			if (ijROI != null
					&& details.manualRoi) { /*
											 ...
											 */
				final double[] tempScaledImage = Arrays.copyOf(softScaledImage, softScaledImage.length);
				/*
			...
			try {
				 ...
				 * higher density layer (i.e. skin) on top of fat to enable
				 * finding muscle border
				 */
				byte[] muscleSieve = Arrays.copyOf(softSieve, softSieve.length);
				final double[] muscleImage = Arrays.copyOf(softScaledImage, softScaledImage.length);
				...
				for (int i = 0; i < 3; ++i) {
					...
					// Changed 2016/01/08
					/*
					 * if (i == 0){ //Add subcut fat sieve... Skin has already
					 * been removed by eroding one layer of pixels-> remove
					 * muscle later on subCutaneousFat = (byte[])
					 * muscleSieve.clone(); }
					 */
				}
				...
				int areaToAdd = 0;
				/*
				 * Include areas that contribute more than 1.0% on top of what
				 * is already included
				 */
				...
				/*
				 * ImagePlus tempImage =
				 * NewImage.createByteImage("MuscleSieve",width,height,1,
				 * NewImage.FILL_BLACK); byte[] rPixels =
				 * (byte[])tempImage.getProcessor().getPixels(); for (int i =
				 * 0;i<muscleSieve.length;++i){ for (int c = 0;c<width;++c){
				 * rPixels[i] = muscleSieve[i]; } }
				 * tempImage.setDisplayRange(0,10); tempImage.show();
				 */

				/**
				 * Re-segment soft-tissues using livewire based on the
				 * muscleSieve 1) bring rays back from image edges to centre of
				 * soft-tissue mask 1 deg apart 2) use livewire on the 360 edge
				 * pixels 3) rotate livewire init pixels around a few times to
				 * get the segmentation to go through subcut/intramuscular fat
				 * 
				 */
				...
				final ArrayList<Double> ii = new ArrayList<Double>();
				final ArrayList<Double> jj = new ArrayList<Double>();
				...
				final double maxR = Math.sqrt(Math.pow(max(Math.abs(softCentre[0] - width), softCentre[0]), 2d)
						+ Math.pow(max(Math.abs(softCentre[1] - height), softCentre[1]), 2d));
				final double[] rs = new double[radialDivisions];
				double r, t;
				final double[] theta = new double[radialDivisions];
				final int[][] edgeCoords = new int[radialDivisions][2];
				// Get the extremes of muscle area with 1 deg increments in
				// polar coordinates
				for (int i = 0; i < radialDivisions; ++i) {
					r = maxR;
					t = ((double) i) / ((double) radialDivisions) * 2d * Math.PI;
					while (Math.round(r * Math.cos(t) + softCentre[0]) < 0
							|| Math.round(r * Math.sin(t) + softCentre[1]) < 0
							|| Math.round(r * Math.cos(t) + softCentre[0]) >= width
							|| Math.round(r * Math.sin(t) + softCentre[1]) >= height) {
						r -= 0.1;
					}
					while (Math.round(r * Math.cos(t) + softCentre[0]) >= 0
							&& Math.round(r * Math.sin(t) + softCentre[1]) >= 0
							&& Math.round(r * Math.cos(t) + softCentre[0]) < width
							&& Math.round(r * Math.sin(t) + softCentre[1]) < height
							&& muscleSieve[(int) (Math.round(r * Math.cos(t) + softCentre[0])
									+ Math.round(r * Math.sin(t) + softCentre[1]) * width)] < 1) {
						r -= 0.1;
					}
					rs[i] = r;
					theta[i] = t;
				...
				for (int rr = 0; rr < height; ++rr) {
					for (int c = 0; c < width; ++c) {
						pixels[c][rr] = muscleSieve[c + rr * width];
					}
				}
				...
				final ArrayList<Integer> seedii = new ArrayList<Integer>();
				final ArrayList<Integer> seedjj = new ArrayList<Integer>();

				final double lwSteps = 6;
				...
				final Vector<Object> tempVO = getLassoEdge(edgeii, edgejj, softCentre, muscleSieve);
				edgeii = (ArrayList<Integer>) tempVO.get(0);
				edgejj = (ArrayList<Integer>) tempVO.get(1);

				/*
				 * //Loop livewire 6 times over here for (int l = 0; l< ((int)
				 * lwSteps);++l){//details.edgeDivisions;++l){ //Set new seeds
				 * Vector<Object> tempVO = getSeedPoints(edgeii, edgejj,
				 * softCentre,details.edgeDivisions, lwSteps, (double) l);
				 * seedii = (ArrayList<Integer>) tempVO.get(0); seedjj =
				 * (ArrayList<Integer>) tempVO.get(1);
				 * 
				 * edgeii.clear(); edgejj.clear(); LiveWireCosts lwc = new
				 * LiveWireCosts(pixels); int[][] fromSeedToCursor; int
				 * prevLength = 0; for (int i = 0;i<seedii.size()-1; ++i){
				 * lwc.setSeed(seedii.get(i),seedjj.get(i));
				 * try{Thread.sleep(10);}catch(Exception e){} while
				 * ((fromSeedToCursor =
				 * lwc.returnPath(seedii.get(i+1),seedjj.get(i+1))) == null){
				 * try{Thread.sleep(1);}catch(Exception e){} } for (int iii =
				 * 0;iii< fromSeedToCursor.length;++iii){ edgeii.add((int)
				 * fromSeedToCursor[iii][0]); edgejj.add((int)
				 * fromSeedToCursor[iii][1]); } if (l == (((int) lwSteps)-1)){
				 * //IJ.log("Edge Length "+(edgeii.size()-prevLength)+" x "
				 * +seedii.get(i)+" y "+seedjj.get(i)+" to x "+seedii.get(i+1)
				 * +" y "+seedjj.get(i+1)); } prevLength = edgeii.size();
				 * 
				 * } //Connect the last bit
				 * lwc.setSeed(seedii.get(seedii.size()-1),seedjj.get(seedjj.
				 * size()-1)); try{Thread.sleep(10);}catch(Exception e){} while
				 * ((fromSeedToCursor =
				 * lwc.returnPath(seedii.get(0),seedjj.get(0))) == null){
				 * try{Thread.sleep(1);}catch(Exception e){} }
				 * 
				 * 
				 * for (int i = 0;i< fromSeedToCursor.length;++i){
				 * edgeii.add((int) fromSeedToCursor[i][0]); edgejj.add((int)
				 * fromSeedToCursor[i][1]); } if (l == (((int) lwSteps)-1)){
				 * //IJ.log("Remainder Edge Length "+(edgeii.size()-prevLength)
				 * +" x "+seedii.get(seedii.size()-1)+" y "+seedjj.get(seedii.
				 * size()-1)+" to x "+seedii.get(0)+" y "+seedjj.get(0)); }
				 * 
				 * 
				 * }
				 */
				/*
				 * IJ.log("Seed points"); for (int i = 0; i<seedii.size();++i){
				 * IJ.log(seedii.get(i)+" "+seedjj.get(i)); }
				 * 
				 * IJ.log("Edge points"); for (int i = 0; i<edgeii.size();++i){
				...
				/*
				 * //Visualise the segmentation result ImagePlus muscleImage2 =
				 * NewImage.createByteImage("muscleImage",width,height,1,
				 * NewImage.FILL_BLACK); byte[] rPixels3 =
				 * (byte[])muscleImage2.getProcessor().getPixels(); for (int i =
				 * 0;i<muscleSieve.length;++i){ for (int c = 0;c<width;++c){
				 * rPixels3[i] = muscleSieve[i]; } }
				 * muscleImage2.setDisplayRange(0,1); muscleImage2.show();
				 */

				muscleSieve = dilateMuscleMask(muscleSieve, softScaledImage, width, height, muscleThreshold); // Dilate
																												// the
																												// sieve
																												// to
																												// include
				...
				final byte[] tempMuscleSieve = Arrays.copyOf(muscleSieve, muscleSieve.length);
				...
				for (int i = 0; i < softSieve.length; ++i) {
					...
					if (muscleSieve[i] == 1 && boneResult[i] == 0 && softScaledImage[i] >= muscleThreshold
							&& softScaledImage[i] < softThreshold) {
						softSieve[i] = 3; // Muscle
					}
					if (muscleSieve[i] == 1 && boneResult[i] == 0 && softScaledImage[i] >= airThreshold
				...
				/*
				 * //Visualise the segmentation result ImagePlus softImage =
				 * NewImage.createByteImage("SoftSieve",width,height,1,
				 * NewImage.FILL_BLACK); byte[] rPixels2 =
				 * (byte[])softImage.getProcessor().getPixels(); for (int i =
				 * 0;i<softSieve.length;++i){ for (int c = 0;c<width;++c){
				 * rPixels2[i] = softSieve[i]; } }
				 * softImage.setDisplayRange(0,6); softImage.show();
				 */
	...
	/* Debugging */
	public void addTrace(final ImagePlus tempImage, final ArrayList<Integer> edgeii, final ArrayList<Integer> edgejj) {
		final int[] tempii = new int[edgeii.size()];
		final int[] tempjj = new int[edgeii.size()];
		for (int i = 0; i < edgeii.size(); ++i) {
			tempii[i] = edgeii.get(i);
			tempjj[i] = edgejj.get(i);
		}

		final Polygon polygon = new Polygon(tempii, tempjj, tempii.length);
		/* Create the ROI */
	...
	/*
	 * Get the line coordinates from origin to target Digital Differential
	 * Analyzer (DDA) algorithm for line
	 * http://www.tutorialspoint.com/computer_graphics/line_generation_algorithm
	 * .htm
	 */
	private ArrayList<Coordinate> getLine(final Coordinate origin, final Coordinate target) {
		final Coordinate difference = target.subtract(origin);
		final double steps = difference.maxVal();
		final double[] increments = new double[] { difference.ii / steps, difference.jj / steps };
		final ArrayList<Coordinate> coordinates = new ArrayList<Coordinate>();
		coordinates.add(origin);
		for (int i = 0; i < (int) steps; ++i) {
			coordinates.add(new Coordinate(coordinates.get(coordinates.size() - 1).ii + increments[0],
					coordinates.get(coordinates.size() - 1).jj + increments[1]));
		}
		// round
	...
	public Vector<Object> getLassoEdge(final ArrayList<Integer> edgeii, final ArrayList<Integer> edgejj,
			final double[] softCentre, final byte[] image) {
		// IJ.log("Lasso");
		final int sectorToConsider = (int) (details.edgeDivisions / 360d * (radialDivisions));
		/*
		 * //Visualise the segmentation result ImagePlus muscleImage2 =
		 * NewImage.createByteImage("muscleImage",width,height,1,
		 * NewImage.FILL_BLACK); byte[] rPixels3 =
		 * (byte[])muscleImage2.getProcessor().getPixels(); for (int i =
		 * 0;i<image.length;++i){ rPixels3[i] = image[i]; }
		 * muscleImage2.setDisplayRange(0,10); muscleImage2.show();
		 * //addTrace(muscleImage2,edgeii,edgejj);
		 */

		// Pop edge into DetectedRadialEdgeTheta, sort by incrementing radius
		// Start the algorithm from the most distant point from the centre of
		// area
		final Vector<DetectedRadialEdgeTheta> radialEdge = new Vector<DetectedRadialEdgeTheta>();
		final double theta;
		final double r;
		double ii, jj;
		for (int i = 0; i < edgeii.size(); ++i) {
			...
			radialEdge.add(new DetectedRadialEdgeTheta(edgeii.get(i), edgejj.get(i), Math.atan2(ii, jj),
					Math.sqrt(Math.pow(ii, 2d) + Math.pow(jj, 2d)), i));
		}
		...
		// IJ.log("maxIndex "+maxIndex+" r
		// "+(radialEdge.get(radialEdge.size()-1).radius/2));
		final int[] indices = new int[radialDivisions];
		int count = 0;
		...
		int currentI = 0;
		final ArrayList<Integer> boundaryIndices = new ArrayList<Integer>();
		final ArrayList<Integer> currentIndices = new ArrayList<Integer>();
		// Loop through the boundary pixels. Look for the furthest point which
		// can be seen
		// From the current point by checking if a line between the points has
		// any roi
		// points in it
		boundaryIndices.add(0);
		while (currentI < (radialDivisions - 1)) {
			// IJ.log("Start of loop "+currentI);
			// Go through all point pairs from the furthest, select the first,
			// which
			...
			int sightIndice = 1;
			// IJ.log("Checking "+currentI+" currentIndices.size()
			// "+currentIndices.size());
			for (int i = currentIndices.size() - 1; i > 0; --i) {
				final ArrayList<Coordinate> tempCoordinates = getLine(
						new Coordinate(edgeii.get(indices[currentI]), edgejj.get(indices[currentI])),
						new Coordinate(edgeii.get(indices[currentIndices.get(i)]),
								edgejj.get(indices[currentIndices.get(i)])));
				// IJ.log("Line x "+tempCoordinates.get(t).ii+" y
				// "+tempCoordinates.get(t).jj+" val "+image[(int)
				// (tempCoordinates.get(t).ii+tempCoordinates.get(t).jj*width)]);
				/*
				 * if (currentI >330){ for (int t =
				 * 0;t<tempCoordinates.size();++t){
				 * IJ.log("Line x "+tempCoordinates.get(t).ii+" y "
				 * +tempCoordinates.get(t).jj+" val "+image[(int)
				 * (tempCoordinates.get(t).ii+tempCoordinates.get(t).jj*width)])
				 * ; } }
				 */
				final int lineOfSight = checkPath(image, tempCoordinates); // 1
																			// =
																			// path
																			// unblocked,
																			// 0
		 ...
		 * IJ.log("Boundary coordinates i "+i+" x "+(edgeii.get(indices[
		 * boundaryIndices.get(i)])/2)+" y "+(edgejj.get(indices[boundaryIndices
		 * .get(i)])/2)); }
		 */

		// Create the boundary
		final ArrayList<Integer> returnii = new ArrayList<Integer>();
		final ArrayList<Integer> returnjj = new ArrayList<Integer>();
		for (int i = 1; i < boundaryIndices.size(); ++i) {
			final ArrayList<Coordinate> tempCoordinates = getLine(
					new Coordinate(edgeii.get(indices[boundaryIndices.get(i - 1)]),
							edgejj.get(indices[boundaryIndices.get(i - 1)])),
					new Coordinate(edgeii.get(indices[boundaryIndices.get(i)]),
							edgejj.get(indices[boundaryIndices.get(i)])));
		...
		final ArrayList<Coordinate> tempCoordinates = getLine(
				new Coordinate(edgeii.get(indices[boundaryIndices.get(boundaryIndices.size() - 1)]),
						edgejj.get(indices[boundaryIndices.get(boundaryIndices.size() - 1)])),
				new Coordinate(edgeii.get(indices[boundaryIndices.get(0)]),
						edgejj.get(indices[boundaryIndices.get(0)])));
		for (int j = 1; j < tempCoordinates.size(); ++j) {
			returnii.add((int) tempCoordinates.get(j).ii);
			returnjj.add((int) tempCoordinates.get(j).jj);
		}
		...
		final Vector<Object> returnV = new Vector<Object>();
	...
	public byte checkPath(final byte[] image, final ArrayList<Coordinate> pathCoordinates) {
		int blocked = 0;
	...
	public Vector<Object> getSeedPoints(final ArrayList<Integer> edgeii, final ArrayList<Integer> edgejj,
			final double[] softCentre, final double divisions, final double steps, final double l) {
		final ArrayList<Integer> seedii = new ArrayList<Integer>();
		final ArrayList<Integer> seedjj = new ArrayList<Integer>();

		// Pop edge into DetectedRadialEdge, sort by incrementing radius
		final Vector<DetectedRadialEdge> radialEdge = new Vector<DetectedRadialEdge>();
		final double theta;
		final double r;
		double ii, jj;
		for (int i = 0; i < edgeii.size(); ++i) {
			ii = edgeii.get(i) - softCentre[0];
			jj = edgejj.get(i) - softCentre[1];
			radialEdge.add(new DetectedRadialEdge(edgeii.get(i), edgejj.get(i), Math.atan2(ii, jj),
					Math.sqrt(Math.pow(ii, 2d) + Math.pow(jj, 2d))));
		}
		Collections.sort(radialEdge);
		...
		for (int i = (int) (l * ((radialEdge.size()) / divisions) / steps); i < (int) (radialEdge.size()
				- ((radialEdge.size()) / divisions)); i += ((int) ((radialEdge.size()) / divisions))) {

			// Look for the furthest point in this bracket
			maxInd = i;
			tempR = radialEdge.get(i).radius;
			for (int j = i + 1; j < i + ((int) ((edgeii.size()) / divisions)); ++j) {
		...
		final Vector<Object> returnVal = new Vector<Object>();
	...
	public byte[] dilateMuscleMask(final byte[] mask, final double[] softScaledImage, final int width, final int height,
			final double threshold) {
		final ArrayList<Integer> initialI = new ArrayList<Integer>();
		final ArrayList<Integer> initialJ = new ArrayList<Integer>();
		int i, j;
		for (i = 0; i < width; ++i) {
			for (j = 0; j < height; ++j) {
				if (mask[i + j * width] == 1) {
		...
		while (initialI.size() > 0 && initialI.get(initialI.size() - 1) > 0
				&& initialI.get(initialI.size() - 1) < width - 1 && initialJ.get(initialJ.size() - 1) > 0
				&& initialJ.get(initialJ.size() - 1) < height - 1) {
	...
	byte[] getByteMask(final int width, final int height, final ArrayList<Integer> edgeii,
			final ArrayList<Integer> edgejj) {
		final byte[] mask = new byte[width * height];
		for (int i = 0; i < edgeii.size(); ++i) {
			mask[edgeii.get(i) + edgejj.get(i) * width] = (byte) 1;
		}
		final int[] fillInitCoords = findMaskFillInit(mask, width, height, edgeii, edgejj);
	...
	byte[] fillMask(int i, int j, final byte[] mask, final int width, final int height) {
		final ArrayList<Integer> initialI = new ArrayList<Integer>();
		final ArrayList<Integer> initialJ = new ArrayList<Integer>();
		initialI.add(i);
		initialJ.add(j);
	...
	int[] findMaskFillInit(final byte[] mask, final int width, final int height, final ArrayList<Integer> edgeii,
			final ArrayList<Integer> edgejj) {
	...
	byte[] fillBorder(final byte[] mask, final int width, final int height) {
		final ArrayList<Integer> initialI = new ArrayList<Integer>();
		final ArrayList<Integer> initialJ = new ArrayList<Integer>();
		initialI.add(0);
		initialJ.add(0);
