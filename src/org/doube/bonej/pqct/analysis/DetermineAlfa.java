	...
	public DetermineAlfa(final SelectROI roi, final ImageAndAnalysisDetails details) {
		...
		if (details.rotationChoice.equals(details.rotationLabels[1])) {
			...
			if (roi.boneMarrowRoiI.size() > 0) {
				...
				marrowCenter[0] /= roi.cortexAreaRoiI.size();
				marrowCenter[1] /= roi.cortexAreaRoiJ.size();
			}
		...
		if (details.rotationChoice.equals(details.rotationLabels[3])
			...
			try {
				...
				/*
				 * Find the second biggest bone (could be bigger than the
				 * selected roi...
				 */
				...
				if (details.rotationChoice.equals(details.rotationLabels[4])) {
					x = selectedBoneCenter[0] - otherBoneCenter[0]; // Use the
																	// other
																	// bone as
																	// origin
																	// for
																	// rotation
					y = selectedBoneCenter[1] - otherBoneCenter[1]; // Use the
																	// other
																	// bone as
																	// origin
																	// for
